...
if __name__ == '__main__':
    ...
    parser.add_argument('--number_workers', '-nw', '--num_workers', type=int, default=0)
    parser.add_argument('--number_gpus', '-ng', type=int, default=-1, help='number of GPUs to use')
    parser.add_argument('--no_cuda', action='store_true')
    #
    parser.add_argument('--seed', type=int, default=1)
    parser.add_argument('--name', default='run', type=str, help='a name to append to the save directory')
    parser.add_argument('--save', '-s', default='checkpoints/shanhgaitech/', type=str, help='directory for saving')
    ...
    parser.add_argument('--resume', default='../flownet2-pytorch/pretrained/FlowNet2-SD_checkpoint.pth.tar', type=str, metavar='PATH',
    ...
    tools.add_arguments_for_module(parser, models, argument_for_class='model', default='FlowNet2SD')
    #
    tools.add_arguments_for_module(parser, losses, argument_for_class='loss', default='MultiScale')

    tools.add_arguments_for_module(parser, torch.optim, argument_for_class='optimizer', default='Adam',
                                   skip_params=['params'])

    tools.add_arguments_for_module(parser, datasets, argument_for_class='training_dataset', default='ChairsSDHom',
                                   skip_params=['is_cropped'],
                                   parameter_defaults={'root': '../../datasets/ChairsSDHom/data/'})
    ...
    # #                                                    'replicates': 1})
    #
    tools.add_arguments_for_module(parser, datasets, argument_for_class='inference_dataset', default='ChairsSDHom',
                                   skip_params=['is_cropped'],
                                   parameter_defaults={'root': '../../datasets/ChairsSDHom/data/',
                                                       'replicates': 1})
    ...
    def train(args, epoch, start_iteration, data_loader, model, optimizer, is_validate=False, offset=0):
        statistics = []
        total_loss = 0
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            # print(data[0].min(), data[0].max(), target[0].min(),target[0].max())
