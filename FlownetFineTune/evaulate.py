...
def load_cosSim_gt(loss_file):
    ...
    css_records = results['score']

    num_videos = len(css_records)
    ...
    return dataset, css_records, gt




def load_css(loss_file):
    """
...
def get_scores_labels(loss_file):
    ...
    dataset, css_records, gt = load_cosSim_gt(loss_file=loss_file)
...
def precision_recall_auc(loss_file):
    ...
    for sub_loss_file in loss_file_list:
        ...
        print("Shape of thresholds: ", len(thresholds))
        auc = metrics.auc(recall, precision)

        print("AUC Matrics ", auc)
...
def compute_auc(loss_file):
    ...
    for sub_loss_file in loss_file_list:
        # the name of dataset, loss, and ground truth
        dataset, css_records, gt = load_cosSim_gt(loss_file=sub_loss_file)
    ...
    return optimal_results #, optimal_roc

def compare_roc(loss_dir):
    loss_path_list = glob.glob(loss_dir+'/*')
    plt.figure()
    for path in loss_path_list:
        _, optimal_roc = compute_auc(path)
        fpr, tpr = optimal_roc
        plt.plot(fpr, tpr,label=path.split('_')[-1])
        plt.title('ShanghaiTech')
        plt.xlabel('fpr')
        plt.ylabel('tpr')
...
def average_css(loss_file):
    ...
    for file in loss_file_list:
        css_records = load_css(file)

        css_records = np.concatenate(css_records, axis=0)
        avg_css = np.mean(css_records)
        if max_avg_css < avg_css:
            max_avg_css = avg_css
            max_file = file
        print('{}, average Cos Sim. = {}'.format(file, avg_css))

    print('max average css file = {}, css = {}'.format(max_file, max_avg_css))


def calculate_css(loss_file):
    optical_result, _ = compute_auc(loss_file)
    print('##### optimal result and model = {}'.format(optical_result))

    mean_css = []
    for file in os.listdir(loss_file):
        file = os.path.join(loss_file, file)
        dataset, css_records, gt = load_cosSim_gt(file)

        css_records = np.concatenate(css_records, axis=0)
        gt = np.concatenate(gt, axis=0)

        mean_normal_css = np.mean(css_records[gt == 0])
        mean_abnormal_css = np.mean(css_records[gt == 1])
        mean = np.mean(css_records)
        print('mean normal css = {}, mean abnormal css = {}, mean = {}'.format(
            mean_normal_css,
            mean_abnormal_css,
            mean)
        )
        mean_css.append(mean)
    print('max mean css = {}'.format(np.max(mean_css)))
...
def calculate_score(loss_file):
    if not os.path.isdir(loss_file):
        ...
        print('##### optimal result and model = {}'.format(optical_result))
    dataset, css_records, gt = load_cosSim_gt(loss_file=loss_file_path)
...
def test_func(*args):
    ...
    for i in range(num_videos):
        ...
        simulated_score.append(np.random.random(size=sub_video_length))

    simulated_results['score'] = simulated_score
    ...
    result, _ = compute_auc(file_path.name)
...
eval_type_function = {
    ...
    'calculate_css': calculate_css,
    'calculate_score': calculate_score,
    'average_css': average_css,
    'average_css_sample': average_css
}
...
if __name__ == '__main__':
    ...
    elif eval_type == 'compare_roc':
        compare_roc(file_path)
    else:
