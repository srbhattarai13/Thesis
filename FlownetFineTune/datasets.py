...
class MpiSintel(data.Dataset):
    def __init__(self, args, is_cropped=False, root='', dstype='clean', replicates=1):
        ...
        file_list = sorted(glob(join(flow_root, '*/*.pfm')))
...
class ChairsSDHom(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print('flow size before crop:', flow.shape)
        ...
        # print('flow size before crop:', flow.size())
...
class ImagesFromFolder(data.Dataset):
    def __init__(self, args, is_cropped, root='/path/to/frames/only/folder', iext='jpg', replicates=1):
        ...
        images = []
        for video in vids:
            for img in (os.listdir(root + str(video))):
                images.append(os.path.join(root + str(video), img))
        # print(images[3])
        self.image_list = []
        ...
        # print('renger_size', self.render_size)
