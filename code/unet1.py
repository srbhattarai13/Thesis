...
class Unet(nn.Module):
    def __init__(self, pool_size=2, kernel_size=3):
        ...
        #downsampple
        self.downconv1_l1 = nn.Conv2d(8, 64, kernel_size=kernel_size, stride=1, padding=1)
    ...
    def forward(self, x):
        x  = self.downconv1_l1(x)
        x = self.relu(x)
        c1 = self.downconv2_l1(x)
        x  = self.dmaxpool(c1)

        x  = self.downconv1_l2(x)
        c2 = self.downconv2_l2(x)
        x  = self.dmaxpool(c2)

        x  = self.downconv1_l3(x)
        c3 = self.downconv2_l3(x)
        x  = self.dmaxpool(c3)

        x = self.downconv1_l4(x)
        x = self.downconv2_l4(x)
        x = self.deconv_l1(x)
