...
models_list = sorted(os.listdir('checkpoints/avenue/4numhis_tanh/'))
# models_list = ['NET_batch_10_epoch_0_0.0001MSE_4numHis.pt']
...
for model in models_list:
    ...
    print(model)
    mdl = torch.load('checkpoints/' + dataset + '/4numhis_tanh/' + model)
    mdl.eval()
    mdl.cuda()
    ...
    with open('results/'+ dataset + '/4numhis_tanh/tanh_result_' + str(model).strip('.pt') , 'wb') as writer:
        pickle.dump(testResult, writer, pickle.HIGHEST_PROTOCOL)

    # # auc = []
    results = evaluate.evaluate('compute_auc', 'results/' + dataset + '/4numhis_tanh/tanh_result_' + str(model).strip('.pt'))
    # # auc.append(results)

    print(results)

    # # auc = { 'model': str(model).strip('.pt'), 'auc': results}
    with open('data/test_results_4numHis_tanh.txt', 'a') as f:
