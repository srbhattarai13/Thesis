...
models_list = sorted(os.listdir('checkpoints/avenue/4numhis/'))
...
for model in models_list:
    ...
    print(model)
    mdl = torch.load('checkpoints/' + dataset + '/4numhis/' + model)
    mdl.eval()
    mdl.cuda()
    ...
    with open('results/'+ dataset + '/4NumHis_MSE_cr/result_' + str(model).strip('.pt') , 'wb') as writer:
        pickle.dump(testResult, writer, pickle.HIGHEST_PROTOCOL)

    # auc = []
    results = evaluate.evaluate('compute_auc', 'results/' + dataset + '/4NumHis_MSE_cr/result_' + str(model).strip('.pt'))
    # auc.append(results)

    print(results)

    # auc = { 'model': str(model).strip('.pt'), 'auc': results}
    with open('data/test_results_4numHis_cr.txt', 'a') as f:
